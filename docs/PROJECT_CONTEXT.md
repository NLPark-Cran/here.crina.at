...
✅ R6 文档处理：documents 表 + POST /api/docs/upload（PDF mutool/DOCX python-docx/图片 qwen3-vl-plus 视觉转写，≤10MB，提取≤2万字）+ list/detail/delete + POST /api/docs/export（md→docx python-docx / md→pdf reportlab CID 字体 STSong-Light，OTC CFF 字体 reportlab 不支持是坑）+ 聊天📎引用（SendMessage.doc_ids→load_doc_context 拼进当轮上下文，材料铁律 prompt 防臆想）+ 委托引用（CreateTask.doc_ids 拼进 prompt）；Board 新增「我的文档」区块、轻IDE 导出 docx/pdf 按钮
✅ 思考强度：探讨模式（brainstorm/guide/probe/extract）开 enable_thinking + thinking_budget=8192（config.chat_thinking_budget 可调）；auto/off 闲聊不思考保秒回活人感；Gateway 支持 reasoning_content/thinking_budget 已实测
📋 待做（按序）：R7 博客与房间(articles+点赞反应收藏+crina日报)
