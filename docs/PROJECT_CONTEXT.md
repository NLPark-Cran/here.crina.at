...
📋 待做：**R8 活人感（DayPlan 全天剧本 7 居民全量 day_plans 表+蛐蛐 <aside> 仅私聊+情感日记 diaries 表 7 居民+mood_note 行为指引+世界观设定集 docs/soul/）→ R9 记忆（守门员+topK 四选一恒定成本写入/防自我强化+主客体过滤/关联图两跳 links JSONB/摘抄即记忆）→ R10 工程（SSE gen_id 断线续流/客厅快照秒开/圆桌脑暴三幕升级）——详见 docs/research/alice-gap.md（Alice 方法论差距分析，已与站主两轮对齐）**；遗留低优先：JWT 黑名单、观猹 secret 轮换、cron 闹钟未装
